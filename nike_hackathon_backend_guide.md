...
![Create new Functions App](resources/portal_create_new_functions_app.png)
...
![New Functions App Settings](resources/portal_create_new_functions_app_settings.png)
1. It can take a few minutes before this process completes 1. check the notifications 
![Notification](resources/portal_notifications_in_progress.png =50%)
...
![New Cosmos DB](resources/portal_create_new_cosmos_database.png)
...
![New Cosmos DB Settings](resources/portal_create_new_cosmos_database_settings.png)
1. It can take a few minutes before this process completes 1. check the notifications 
...
1. Use Postman or a browser to make a GET request to the function (e.g. `http://localhost:7071/api/Function1?name=Rob`) and verify the ouput `Hello, Rob`
...
1. Copy the site URL and verify the function is running by running that same GET request against the remote instance (e.g. `http://myfunctionsapp.azurewebsites.net/api/Function1?name=Rob`) and verify the ouput `Hello, Rob`
...
1. Copy the "Blob service SAS URL" to somewhere safe (e.g. `https://mynewstorageaccountblob.blob.core.windows.net/?sv=2017-04-17&ss=b&srt=sco&sp=rwdlac&se=2018-01-05T23:03:17Z&st=2018-01-04T23:03:17Z&spr=https&sig=HIE2i2WSliZkiQ8lAw0qjykZm1VbywZdAGbUhrWfl%2BI%3D`)
...
### Step 10: Make a prediction with Custom Vision and log the results
...
1. Build and run locally and verify using teh
1. Use Postman to make the same local POST request as in the previous step
1. Verify the data was inserted by inspecting the Prediction JSON object that is returned.
1. Publish to the cloud and re-verify


### Step 11: Connect the mobile front-end to the Functions backend
...
1. Right-click on either the iOS Android project and select "Rebuild"
...
1. Select an emulator from the drop-down list and click the Start Debug button
  1. It can take several minutes to comp



### Step 12: Add another function that retrieves a list of past predictions
